...
import { runProfileAgent } from "../agents/profileAgent.ts";
...
export async function handleCommand(input: string, context: SessionContext): Promise<UIMessage> {
  ...
  if (context.pendingAgent === "profile") {
    return runProfileAgent(trimmed, context.recentMessages);
  }
  ...
  if (trimmed === "/profile" || trimmed === "/profile setup") {
    return runProfileAgent(
      trimmed === "/profile setup" ? "Set up my profile" : "Show my current profile",
      []
    );
  }
...
File contents:
${content}`;
    return runGoalCoachAgent(prompt);
  }
